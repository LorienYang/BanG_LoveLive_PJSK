...
const CONFIG = {
  BASE_WIDTH: 960,
  BASE_HEIGHT: 700,
  API_URL: 'https://mapapi.enldm.cyou/api/lovelive'
};
...
function updateThemeMetaColor(theme) {
  ...
  metaThemeColor.setAttribute('content', theme === 'dark' ? '#2b1a05' : '#f6a623');
  ...
  if (supportsDynamicThemeColor) {
    document.documentElement.style.setProperty('background-color', theme === 'dark' ? '#2b1a05' : '#fff8ee');
    document.body.style.setProperty('background-color', theme === 'dark' ? '#2b1a05' : '#fff8ee');
  }
}
...
function renderCurrentDetail() {
  ...
  const sourceRows = State.globalSearchEnabled ? State.projectRows : State.currentDetailRows;
...
function updateSummaryUI(source, animate = true) {
  const applySummary = () => {
    ...
    document.getElementById('selectedTitle').textContent = '全国拉群数据';
...
function placeMapBubble(anchorX, anchorY) {
  ...
  const currentTransform = d3.zoomTransform(State.mapViewState.svg.node());
  bubble.style.left = `${currentTransform.x + anchorX * currentTransform.k}px`;
  bubble.style.top = `${currentTransform.y + anchorY * currentTransform.k}px`;
}
...
const MapUtils = {
  colorByCount: (count, maxCount) => {
    if (!count) return '#d9f3ff';
    const ratio = Math.max(0, Math.min(1, count / Math.max(1, maxCount)));
    return ratio > 0.75 ? '#0288d1' : ratio > 0.5 ? '#1ea6e8' : ratio > 0.25 ? '#5bc5f2' : '#8ddcff';
  },
...
function renderChinaMap() {
  ...
  china().width(w).height(h).scale(1).language('cn').colorDefault('#d9f3ff').colorLake('#ffffff').draw('#mapSvg');
  ...
  svg.call(zoom)
    .call(zoom.transform, d3.zoomIdentity.translate(offsetX, offsetY).scale(fitScale))
...
async function reloadBandoriData() {
  ...
  try {
    const resp = await fetch(CONFIG.API_URL, { cache: 'no-store' });
    if (resp.ok) {
...
function bindAllStaticEvents() {
  ...
  const stepScale = (factor) => {
    ...
    const nextTransform = d3.zoomIdentity
      ...
      .scale(nextScale)
      .translate(
        -(center[0] - currentTransform.x) / currentTransform.k,
        -(center[1] - currentTransform.y) / currentTransform.k
      );
