...
const CONFIG = {
  BASE_WIDTH: 960,
  BASE_HEIGHT: 700,
  API_URL: 'https://mapapi.enldm.cyou/api/pjsk'
};
...
function updateThemeMetaColor(theme) {
  ...
  metaThemeColor.setAttribute('content', theme === 'dark' ? '#081a22' : '#4fc3f7');
  ...
  if (supportsDynamicThemeColor) {
    document.documentElement.style.setProperty('background-color', theme === 'dark' ? '#081a22' : '#f2fbff');
    document.body.style.setProperty('background-color', theme === 'dark' ? '#081a22' : '#f2fbff');
  }
}
...
function updateSummaryUI(source, animate = true) {
  const applySummary = () => {
    ...
    document.getElementById('selectedTitle').textContent = '全国烤群数据';
